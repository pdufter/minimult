    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.invert_order:
                modifications.invert_pairs(batch[0], model.config.shifts, args, tokenizer)
            if args.language_specific_positions:
                ...
                position_ids, segment_ids = modifications.get_language_specific_positions(batch[0], model.config.shifts, args.block_size, tokenizer)
                position_ids = position_ids.to(args.device)
                segment_ids = segment_ids.to(args.device)
            else:
                position_ids, segment_ids = None, None
            if args.shift_special_tokens:
                modifications.shift_special_tokens(batch[0], model.config.shifts, args.special_token_indices, tokenizer)
            else:
                modifications.unshift_special_tokens(batch[0], model.config.shifts, args.special_token_indices, tokenizer)
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], "position_ids": position_ids, "token_type_ids": segment_ids}
...
def evaluate(args, model, tokenizer, tokenizer_langs, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer_langs[args.language_id.index(args.language)], evaluate=True)
        # now shift dataset
        eval_dataset.tensors = list(eval_dataset.tensors)
        eval_dataset.tensors[0] += model.config.shifts[args.language_id.index(args.language)]
        ...
        with open(output_eval_file, "a") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                writer.write("{} {} {} {} {}\n".format(args.output_dir, args.language, args.seed, key, str(result[key])))
...
def main():
    ...
    parser.add_argument(
            ...
            type=str,
            required=False,
            help="Outputfile for evaluation.",
    )
    ...
    parser.add_argument("--use_dev_for_train", action="store_true", help="")

    parser.add_argument("--replacement_probs", default="0.8,0.5", type=str, help="")
    parser.add_argument("--do_not_replace_with_random_words", action="store_true", help="")
    parser.add_argument("--replace_with_nn", type=int, default=-1, help="")
    parser.add_argument("--vecmap", default=None, type=str, help="")
    ...
    if args.tokenizer_name:
        ...
        for tokenizer_name in args.tokenizer_name:
            tokenizer_langs.append(tokenizer_class.from_pretrained(tokenizer_name, cache_dir=args.cache_dir))# , config=config))
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer_langs[args.language_id.index(args.language)], evaluate=False or args.use_dev_for_train)
        # now shift dataset
        train_dataset.tensors = list(train_dataset.tensors)
        train_dataset.tensors[0] += config.shifts[args.language_id.index(args.language)]
        train_dataset.tensors = tuple(train_dataset.tensors)
